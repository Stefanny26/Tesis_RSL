# ENCUESTA DE VALIDACIÓN: SISTEMA RSL CON IA PARA PRISMA 2020

## INFORMACIÓN GENERAL

**Objetivo:** Evaluar la usabilidad, aceptabilidad y utilidad del sistema de gestión de revisiones sistemáticas de literatura con asistencia de inteligencia artificial.

**Duración estimada:** 15-20 minutos

**Participantes objetivo:** Investigadores, docentes o estudiantes de posgrado con experiencia en revisiones sistemáticas de literatura y conocimiento del estándar PRISMA 2020.

---

## SECCIÓN 1: INFORMACIÓN DEMOGRÁFICA

**1.1 Género**
- [ ] Masculino
- [ ] Femenino
- [ ] Prefiero no especificar
- [ ] Otro: ___________

**1.2 Edad**
- [ ] 18-25 años
- [ ] 26-35 años
- [ ] 36-45 años
- [ ] 46-55 años
- [ ] Más de 55 años

**1.3 Nivel de formación académica**
- [ ] Estudiante de pregrado
- [ ] Estudiante de maestría
- [ ] Estudiante de doctorado
- [ ] Magíster
- [ ] Doctor/PhD
- [ ] Postdoctorado

**1.4 Área de especialización**
- [ ] Ciencias de la Computación / Ingeniería de Software
- [ ] Ciencias de la Salud
- [ ] Ciencias Sociales
- [ ] Educación
- [ ] Ingeniería
- [ ] Otra: ___________

**1.5 Años de experiencia en investigación**
- [ ] Menos de 1 año
- [ ] 1-3 años
- [ ] 4-6 años
- [ ] 7-10 años
- [ ] Más de 10 años

**1.6 ¿Ha realizado revisiones sistemáticas de literatura previamente?**
- [ ] No, es mi primera vez
- [ ] Sí, 1 revisión
- [ ] Sí, 2-3 revisiones
- [ ] Sí, 4-5 revisiones
- [ ] Sí, más de 5 revisiones

**1.7 ¿Está familiarizado con el estándar PRISMA 2020?**
- [ ] No, no lo conozco
- [ ] He escuchado sobre él pero no lo he usado
- [ ] Lo conozco y lo he usado parcialmente
- [ ] Lo conozco bien y lo he aplicado completamente
- [ ] Soy experto en PRISMA 2020

**1.8 ¿Ha utilizado herramientas de inteligencia artificial para investigación académica previamente?**
- [ ] No, nunca
- [ ] Sí, asistentes de escritura (como ChatGPT)
- [ ] Sí, herramientas de análisis de literatura
- [ ] Sí, múltiples herramientas de IA para investigación
- [ ] Otra: ___________

---

## SECCIÓN 2: USABILIDAD DEL SISTEMA (SUS - System Usability Scale)

**Instrucciones:** Para cada afirmación, indique su nivel de acuerdo del 1 al 5:
- 1 = Totalmente en desacuerdo
- 2 = En desacuerdo
- 3 = Neutral (ni de acuerdo ni en desacuerdo)
- 4 = De acuerdo
- 5 = Totalmente de acuerdo

| # | Afirmación | 1 | 2 | 3 | 4 | 5 |
|---|------------|---|---|---|---|---|
| 2.1 | Creo que me gustaría utilizar este sistema frecuentemente | [ ] | [ ] | [ ] | [ ] | [ ] |
| 2.2 | Encontré el sistema innecesariamente complejo | [ ] | [ ] | [ ] | [ ] | [ ] |
| 2.3 | Pensé que el sistema era fácil de usar | [ ] | [ ] | [ ] | [ ] | [ ] |
| 2.4 | Creo que necesitaría apoyo técnico o capacitación para usar este sistema | [ ] | [ ] | [ ] | [ ] | [ ] |
| 2.5 | Encontré que las diversas funciones del sistema estaban bien integradas | [ ] | [ ] | [ ] | [ ] | [ ] |
| 2.6 | Pensé que había demasiada inconsistencia en este sistema | [ ] | [ ] | [ ] | [ ] | [ ] |
| 2.7 | Imagino que la mayoría de investigadores o docentes aprenderían a usar este sistema rápidamente | [ ] | [ ] | [ ] | [ ] | [ ] |
| 2.8 | Encontré el sistema muy engorroso de usar | [ ] | [ ] | [ ] | [ ] | [ ] |
| 2.9 | Necesitaría aprender muchas cosas antes de poder usar este sistema | [ ] | [ ] | [ ] | [ ] | [ ] |

---

## SECCIÓN 3: EVALUACIÓN DE FUNCIONALIDADES ESPECÍFICAS

**Instrucciones:** Evalúe cada funcionalidad según su experiencia de uso.

**Escala:**
- 1 = Muy difícil/inútil
- 2 = Difícil/poco útil
- 3 = Neutral
- 4 = Fácil/útil
- 5 = Muy fácil/muy útil
- N/A = No usé esta función

### 3.1 MÓDULO 1: Protocolo de Investigación (Fase 1 - PICO)

| Funcionalidad | Facilidad de uso | Utilidad |
|---------------|------------------|----------|
| Definición de matriz ES/NO ES | 1 2 3 4 5 N/A | 1 2 3 4 5 N/A |
| Formulación de PICO (Población, Intervención, Comparación, Outcomes) | 1 2 3 4 5 N/A | 1 2 3 4 5 N/A |
| Generación automática de títulos con IA | 1 2 3 4 5 N/A | 1 2 3 4 5 N/A |
| Generación de cadenas de búsqueda para bases de datos | 1 2 3 4 5 N/A | 1 2 3 4 5 N/A |
| Validación de protocolo según estándares | 1 2 3 4 5 N/A | 1 2 3 4 5 N/A |

**3.2 ¿Cuál fue la funcionalidad más útil del Módulo 1?**
```
[Respuesta abierta]
```

**3.3 ¿Qué mejoraría del Módulo 1?**
```
[Respuesta abierta]
```

### 3.4 MÓDULO 1: Gestión de Referencias (Fase 2)

| Funcionalidad | Facilidad de uso | Utilidad |
|---------------|------------------|----------|
| Importación de referencias (CSV, BibTeX) | 1 2 3 4 5 N/A | 1 2 3 4 5 N/A |
| Detección automática de duplicados | 1 2 3 4 5 N/A | 1 2 3 4 5 N/A |
| Selección manual de referencias | 1 2 3 4 5 N/A | 1 2 3 4 5 N/A |
| Selección de referencias con ayuda de inteligencia artificial | 1 2 3 4 5 N/A | 1 2 3 4 5 N/A |
| Ordenamiento inteligente de referencias por relevancia | 1 2 3 4 5 N/A | 1 2 3 4 5 N/A |
| Visualización de resultados de screening | 1 2 3 4 5 N/A | 1 2 3 4 5 N/A |

**3.5 ¿La ayuda de inteligencia artificial para seleccionar referencias le permitió tomar mejores decisiones?**
- [ ] Sí, significativamente
- [ ] Sí, moderadamente
- [ ] Neutral / No estoy seguro
- [ ] No, no fue útil
- [ ] No, fue confuso o engañoso

**3.6 Comentarios adicionales sobre el Módulo de Gestión de Referencias:**
```
[Respuesta abierta]
```

### 3.7 MÓDULO 2: Checklist PRISMA 2020

| Funcionalidad | Facilidad de uso | Utilidad |
|---------------|------------------|----------|
| Navegación entre los 27 ítems PRISMA | 1 2 3 4 5 N/A | 1 2 3 4 5 N/A |
| Generación automática de contenido desde protocolo | 1 2 3 4 5 N/A | 1 2 3 4 5 N/A |
| Edición manual de ítems | 1 2 3 4 5 N/A | 1 2 3 4 5 N/A |
| Revisión automática de ítems con inteligencia artificial | 1 2 3 4 5 N/A | 1 2 3 4 5 N/A |
| Visualización de progreso de cumplimiento | 1 2 3 4 5 N/A | 1 2 3 4 5 N/A |

**3.8 ¿La revisión automática de los ítems PRISMA fue precisa?**
- [ ] Muy precisa, confío plenamente en sus evaluaciones
- [ ] Generalmente precisa, con algunas excepciones
- [ ] Moderadamente precisa, requiere revisión humana
- [ ] Poco precisa, no confío en sus evaluaciones
- [ ] No usé esta función

**3.9 ¿Las sugerencias de mejora automáticas le ayudaron a completar mejor los ítems PRISMA?**
- [ ] Sí, fueron muy útiles y específicas
- [ ] Sí, fueron útiles pero generales
- [ ] Fueron confusas o poco claras
- [ ] No, no las tomé en cuenta
- [ ] No usé esta función

**3.10 Comentarios sobre el Módulo PRISMA:**
```
[Respuesta abierta]
```

---

## SECCIÓN 4: EVALUACIÓN DE INTELIGENCIA ARTIFICIAL

**4.1 Confianza en las recomendaciones de IA**

| Aspecto | Nivel de confianza |
|---------|-------------------|
| Generación de títulos de revisión | 1 2 3 4 5 N/A |
| Generación de cadenas de búsqueda | 1 2 3 4 5 N/A |
| Clasificación de referencias (incluir/excluir) | 1 2 3 4 5 N/A |
| Ordenamiento de referencias por relevancia | 1 2 3 4 5 N/A |
| Validación de ítems PRISMA | 1 2 3 4 5 N/A |
| Generación automática de contenido PRISMA | 1 2 3 4 5 N/A |

**Escala:** 1 = No confío en absoluto, 5 = Confío completamente

**4.2 ¿Encontró algún error significativo en las recomendaciones de la IA?**
- [ ] No, todas fueron correctas
- [ ] Sí, errores menores (1-2 casos)
- [ ] Sí, algunos errores (3-5 casos)
- [ ] Sí, muchos errores (más de 5 casos)
- [ ] No presté atención a este aspecto

**4.3 Si encontró errores, ¿de qué tipo fueron? (puede marcar varias)**
- [ ] Clasificación incorrecta de referencias
- [ ] Títulos generados poco coherentes
- [ ] Términos de búsqueda incompletos
- [ ] Revisión PRISMA demasiado estricta
- [ ] Revisión PRISMA demasiado permisiva
- [ ] Sugerencias irrelevantes
- [ ] Otro: ___________

**4.4 ¿El sistema explicó claramente cómo llegaba a sus recomendaciones?**
- [ ] Muy claro, siempre explicó el razonamiento
- [ ] Generalmente claro
- [ ] Neutral
- [ ] Poco claro
- [ ] Nada claro, no entendí cómo decidía

**4.5 ¿Prefiere tener control total sobre las recomendaciones o que el sistema tome decisiones automáticas?**
- [ ] Control total, yo reviso y apruebo todo
- [ ] Control mayoritario, el sistema sugiere pero yo decido
- [ ] Balance 50/50
- [ ] El sistema decide, yo superviso
- [ ] Automatización total, confío en las recomendaciones

---

## SECCIÓN 5: COMPARACIÓN CON MÉTODOS TRADICIONALES

**5.1 ¿Ha realizado revisiones sistemáticas sin ayuda de IA anteriormente?**
- [ ] Sí, varias veces
- [ ] Sí, una vez
- [ ] No, esta es mi primera RSL
- [ ] Participé pero no lideré

**5.2 Comparado con métodos tradicionales (sin IA), este sistema es:**

| Aspecto | Comparación |
|---------|-------------|
| **Velocidad:** Completar protocolo PICO | Mucho más lento [ ] - Igual [ ] - Mucho más rápido [ ] |
| **Velocidad:** Selección de referencias | Mucho más lento [ ] - Igual [ ] - Mucho más rápido [ ] |
| **Velocidad:** Redacción de ítems PRISMA | Mucho más lento [ ] - Igual [ ] - Mucho más rápido [ ] |
| **Calidad:** Precisión del protocolo | Mucho peor [ ] - Igual [ ] - Mucho mejor [ ] |
| **Calidad:** Decisiones de selección de estudios | Mucho peor [ ] - Igual [ ] - Mucho mejor [ ] |
| **Calidad:** Cumplimiento PRISMA | Mucho peor [ ] - Igual [ ] - Mucho mejor [ ] |

**5.3 ¿Cuánto tiempo estimado le ahorró el sistema comparado con hacerlo manualmente?**
- [ ] No ahorró tiempo, fue igual o más lento
- [ ] Ahorró menos de 5 horas
- [ ] Ahorró entre 5-10 horas
- [ ] Ahorró entre 10-20 horas
- [ ] Ahorró entre 20-40 horas
- [ ] Ahorró más de 40 horas
- [ ] No puedo comparar, es mi primera RSL

**5.4 ¿Recomendaría este sistema a colegas que realicen revisiones sistemáticas?**
- [ ] Definitivamente sí
- [ ] Probablemente sí
- [ ] Tal vez / No estoy seguro
- [ ] Probablemente no
- [ ] Definitivamente no

**5.5 ¿Por qué? (Justifique su respuesta anterior)**
```
[Respuesta abierta]
```

---

## SECCIÓN 6: SATISFACCIÓN GENERAL

**6.1 Nivel de satisfacción general con el sistema:**
- [ ] Muy insatisfecho (1)
- [ ] Insatisfecho (2)
- [ ] Neutral (3)
- [ ] Satisfecho (4)
- [ ] Muy satisfecho (5)

**6.2 ¿Qué es lo que MÁS le gustó del sistema?**
```
[Respuesta abierta - mínimo 2 líneas]
```

**6.3 ¿Qué es lo que MENOS le gustó del sistema?**
```
[Respuesta abierta - mínimo 2 líneas]
```

**6.4 ¿Qué funcionalidad agregaría o mejoraría?**
```
[Respuesta abierta]
```

**6.5 ¿Usaría este sistema en su próxima revisión sistemática?**
- [ ] Definitivamente sí, para todo el proceso
- [ ] Sí, para algunas fases específicas
- [ ] Tal vez, dependiendo de mejoras
- [ ] Probablemente no
- [ ] Definitivamente no

---

## SECCIÓN 7: EVALUACIÓN DE DISEÑO E INTERFAZ

**7.1 Diseño visual y estética**
- [ ] Muy deficiente (1)
- [ ] Deficiente (2)
- [ ] Aceptable (3)
- [ ] Bueno (4)
- [ ] Excelente (5)

**7.2 Facilidad de navegación**
- [ ] Muy difícil (1)
- [ ] Difícil (2)
- [ ] Neutral (3)
- [ ] Fácil (4)
- [ ] Muy fácil (5)

**7.3 Claridad de instrucciones y tooltips**
- [ ] Muy confusas (1)
- [ ] Confusas (2)
- [ ] Aceptables (3)
- [ ] Claras (4)
- [ ] Muy claras (5)

**7.4 Tiempo de respuesta del sistema**
- [ ] Muy lento (1)
- [ ] Lento (2)
- [ ] Aceptable (3)
- [ ] Rápido (4)
- [ ] Muy rápido (5)

**7.5 ¿Experimentó errores técnicos durante el uso?**
- [ ] No, ninguno
- [ ] Sí, errores menores que no afectaron mi trabajo
- [ ] Sí, errores moderados que causaron inconvenientes
- [ ] Sí, errores graves que bloquearon mi trabajo
- [ ] Sí, el sistema se cayó o perdí mi trabajo

**7.6 Si experimentó errores, describa brevemente:**
```
[Respuesta abierta]
```

---

## SECCIÓN 8: ACEPTABILIDAD Y ADOPCIÓN

**8.1 ¿Qué tan probable es que adopte herramientas de IA para sus investigaciones futuras?**
- [ ] Muy improbable
- [ ] Improbable
- [ ] Neutral
- [ ] Probable
- [ ] Muy probable

**8.2 ¿Considera que el uso de IA en revisiones sistemáticas compromete la integridad académica?**
- [ ] Sí, definitivamente compromete la integridad
- [ ] Sí, en algunos aspectos
- [ ] Neutral / Depende del uso
- [ ] No, si se usa correctamente
- [ ] No, en absoluto

**8.3 ¿Estaría dispuesto a pagar por una herramienta como esta?**
- [ ] Sí, sin duda
- [ ] Sí, si el precio es razonable (indique rango): ___________
- [ ] Tal vez, dependiendo de funcionalidades
- [ ] No, prefiero herramientas gratuitas
- [ ] No, no veo el valor agregado

**8.4 ¿Qué tan importante es para usted que el sistema sea:**

| Característica | Importancia |
|----------------|-------------|
| Que sea de acceso libre y modificable | Nada [ ] - Poco [ ] - Moderada [ ] - Importante [ ] - Muy importante [ ] |
| Gratuito | Nada [ ] - Poco [ ] - Moderada [ ] - Importante [ ] - Muy importante [ ] |
| Con almacenamiento en la nube | Nada [ ] - Poco [ ] - Moderada [ ] - Importante [ ] - Muy importante [ ] |
| Con opciones de colaboración en equipo | Nada [ ] - Poco [ ] - Moderada [ ] - Importante [ ] - Muy importante [ ] |
| Con exportación a formatos académicos (LaTeX, Word) | Nada [ ] - Poco [ ] - Moderada [ ] - Importante [ ] - Muy importante [ ] |
| Con soporte para múltiples idiomas | Nada [ ] - Poco [ ] - Moderada [ ] - Importante [ ] - Muy importante [ ] |

---

## SECCIÓN 9: COMENTARIOS FINALES

**9.1 ¿Hay algo más que le gustaría compartir sobre su experiencia con el sistema?**
```
[Respuesta abierta - espacio amplio]






```

**9.2 ¿Estaría dispuesto a participar en una entrevista de seguimiento (30 min)?**
- [ ] Sí, con gusto
- [ ] Tal vez, contactarme para confirmar
- [ ] No, gracias

**Si respondió sí, proporcione un correo de contacto (opcional):**
```
Correo: _________________________________
```

---

## CONSENTIMIENTO INFORMADO

**Declaro que:**
- He sido informado/a sobre el propósito de esta encuesta
- Mi participación es voluntaria
- Puedo retirarme en cualquier momento
- Mis respuestas serán tratadas de forma confidencial y usadas únicamente con fines académicos
- Los datos serán anonimizados en reportes y publicaciones

**Acepto participar:** [ ] Sí

**Fecha:** _______________

**Firma (opcional):** _______________

---

## AGRADECIMIENTO

Gracias por su tiempo y valiosa retroalimentación. Sus respuestas contribuirán significativamente al desarrollo y mejora de herramientas de IA para la investigación académica.

**Contacto de investigadores:**
- Hernández Buenaño Stefanny Mishel
- González Orellana Adriana Pamela
- Universidad de las Fuerzas Armadas ESPE
- Departamento de Ciencias de la Computación

---

**Tiempo de completado:** _______ minutos

**Versión de la encuesta:** 1.0 (Enero 2026)
